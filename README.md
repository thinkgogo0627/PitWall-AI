...
> **FastF1 레이스 데이터 · SQLite 하드데이터 · LLM 에이전트를 결합한 Formula 1 분석 도구**

사용자가 시즌 / 그랑프리 / 드라이버를 선택하면 **레이스 브리핑, 전략 분석, 스틴트 차트, 피트 전술 시뮬레이션**을 제공합니다. 공식 결과와 랩 데이터는 SQLite/FastF1 기반으로 처리하고, 뉴스·인터뷰·규정 같은 비정형 근거 검색은 MongoDB·Qdrant RAG가 있을 때 보조로 사용합니다.
...
Streamlit UI는 **Briefing**과 **Strategy Center** 두 탭으로 구성됩니다. 핵심 브리핑/전략 기능은 SQLite와 FastF1 하드데이터를 우선 사용하고, RAG는 심층 질문의 보조 근거로만 사용합니다.

```
Streamlit App
  │
  ├─ Briefing Tab ──────── SQLite(race_results) 직접 조회 → 프롬프트 주입
  │                        + 일반 채팅에서 RAG(뉴스 / 인터뷰 / FIA 규정) 보조
  │
  └─ Strategy Center ───── FastF1 세션 → 스틴트 / 타이어 / 트래픽 / 피트 분석
                           + Head-to-Head 피트 전술 시뮬레이션
```
...
| 벡터 (Vector) | Qdrant (Cloud, optional) | 뉴스·인터뷰·규정 RAG 검색 |
...
- Race Summary / Driver Focus 버튼은 도구 없는 no-tool agent로 실행되어, Qdrant가 없어도 하드데이터 기반 브리핑을 생성합니다.
- 일반 채팅에서 사건 원인·인터뷰·규정 근거가 필요할 때만 RAG 도구를 보조로 호출합니다. 검색 실패 시 추측 없이 "확인되지 않았습니다"로 보고하도록 강제합니다.

### 2. Strategy Agent · `gemini-2.5-flash`
...
- 결과는 `Category / Metrics / Insight / Verdict` JSON 스키마로 정규화되어 Streamlit 테이블로 렌더링됩니다.

### 3. Tactical Simulation Agent · `gemini-2.5-pro`
...
### 4. Telemetry / Analyst Module
- `analytic_agent.py`와 `telemetry_data.py`에는 랩타임 비교, 트랙 도미넌스 맵, 스피드 트레이스 생성 로직이 남아 있습니다.
- 현재 Streamlit 메인 화면은 브리핑과 전략 센터 중심으로 구성되어 있으며, 텔레메트리 분석은 별도 모듈 기능입니다.
...
│   │   ├── analytic_agent.py        # 텔레메트리 시각화 모듈
...
│   └── regulation_tool.py           # FIA 규정 RAG 도구 (Qdrant 필요)
│
├── data_pipeline/
...
QDRANT_URL=your_qdrant_url          # optional: RAG 검색용
QDRANT_API_KEY=your_qdrant_api_key  # optional: RAG 검색용
MONGO_URI=your_mongodb_uri          # optional: 크롤링 원문 저장용
